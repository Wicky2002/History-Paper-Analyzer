...
def build_marking_rubric_table() -> list[dict[str, Any]]:
    """Build a marking rubric table with 5 sample questions and their rubric criteria."""
    rubric_data = [
        {
            "ප්‍රශ්න අංකය": "Q1",
            "ප්‍රශ්නය": "පණ්ඩුකාභය රජතුමාගේ දෙමව්පියන් කවුද?",
            "සම්පූර්ණ ලකුණු": "20",
            "මාපක": "නම් නිවැරදිව ඔක්කොම (20), එක නම් හැරුණු (10), නම් නොපෙන්වූ (0)",
            "ඇගයීම්": "සිංහල ඇගයීමේ මට්ටම: 20/20 නිවැරදි, 10/20 අර්ධ, 0/20 වැරදි"
        },
        {
            "ප්‍රශ්න අංකය": "Q2",
            "ප්‍රශ්නය": "ඉතිහාසයේ සුවිශේෂී අධ්‍යාපනය සඳහා අපි කුමක් කළ යුතුද?",
            "සම්පූර්ණ ලකුණු": "20",
            "මාපක": "පර්යේෂණ ක්‍රම (5), සෝතින්ට්‍රිය ශිල්ප (5), විවරණ කුසලතා (5), අවිධිමත් සිතීම (5)",
            "ඇගයීම්": "20/20 සම්පූර්ණ, 10-15/20 අර්ධ, 0-5/20 අඩුවිය"
        },
        {
            "ප්‍රශ්න අංකය": "Q3",
            "ප්‍රශ්නය": "අනුරාධපුර සම්‍රාජ්‍යයේ සামාජිక ව්‍යවස්තාව කියා දෙන්න.",
            "සම්පූර්ණ ලකුණු": "20",
            "මාපක": "වර්ණ ක්‍රම (5), ජනවර්ගීය සාධන (5), නිවස් ව්‍යවස්තාව (5), ගිණුම් දැමුම (5)",
            "ඇගයීම්": "20/20 සම්පූර්ණ, 12-19/20 හොඳ, 6-11/20 පිරිමැඩුම, 0-5/20 දුර්বල"
        },
        {
            "ප්‍රශ්න අංකය": "Q4",
            "ප්‍රශ්නය": "බුද්ධ ශාසනය ශ්‍රී ලංකාවට ගෙන ආ ක්‍රමය තොරතුරු දෙන්න.",
            "සම්පූර්ණ ලකුණු": "20",
            "මාපක": "කරුණ තුනක් නිවැරදිව (20), කරුණ දෙක නිවැරදිව (15), කරුණ එකක් නිවැරදිව (8), නිවැරදිකම අඩුව (0)",
            "ඇගයීම්": "ඉතිරි අනුපාතයට නිසි ලකුණු: 20, 15, 8, 0"
        },
        {
            "ප්‍රශ්න අංකය": "Q5",
            "ප්‍රශ්නය": "අගබෝධි විස්තරයෙන් පෙන්වන රාජ්‍ය කටයුතු තුන හඳුනා ගන්න.",
            "සම්පූර්ණ ලකුණු": "20",
            "මාපක": "කටයුතු තුනම හඳුනා ගැනීම (20), දෙකක් හඳුනා ගැනීම (13), එකක් හඳුනා ගැනීම (6), කිසිවක් නැත (0)",
            "ඇගයීම්": "ඉතිරි අනුපාතයට සංගතව ලකුණු: 20, 13, 6, 0"
        },
    ]
    return rubric_data
...
def main() -> None:
    ...
    if st.button("Evaluate", use_container_width=True):
        ...
        # Marking Rubric Reference Table
        st.subheader("📊 ඇගයීම් මාපක අවලంబනය - ප්‍රශ්ན 5")
        rubric_rows = build_marking_rubric_table()
        if rubric_rows:
            st.table(rubric_rows)
        else:
            st.info("ඇගයීම් මාපක තොරතුරු නොමැත.")
